...
if (
  context.params.event.content === `${process.env.PREFIX}queue` ||
  context.params.event.content === `${process.env.PREFIX}nowplaying`
) {
...
} else if (context.params.event.content.split(' ')[0] === `${process.env.PREFIX}enqueue`) {
